...
def obter_contas_a_pagar_e_receber_de_um_fornecedor_cliente_por_id(
        id_do_fornecedor_cliente: int,
        db: Session = Depends(get_db)) -> List[ContaPagarReceberResponse]:
    return db.query(ContaPagarReceber).filter_by(fornecedor_cliente_id=id_do_fornecedor_cliente).all()
